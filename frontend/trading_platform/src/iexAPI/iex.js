class Finnhub {
  constructor() {
    ...
    this.token = process.env.REACT_APP_finnhubToken; // Use your Finnhub token from the environment
  }

  // Helper function to build the full URL
  getFullUrl(endpoint, params = {}) {
    // Add token to the query parameters
    ...
    return `${this.prefix}/${endpoint}?${queryString}`;
  }

  // Static method to map endpoint keys to actual API paths
  ...
  // Method to fetch stock quotes (example usage)
  fetchStockQuote(symbol = "AAPL") {
    const url = this.getFullUrl(Finnhub.getInfixUrls("stock"), { symbol });
...
finnhub.fetchStockQuote('AAPL').then(data => console.log(data));
